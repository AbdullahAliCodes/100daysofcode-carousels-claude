# @uncledullaz Carousel System — Context v7

**Golden reference file:** `day-47.2/day47.2_carousel.html`  
**Current project:** Amazon Clone (react-router-dom)  
**Last updated:** Day 47.2
...
0-claude-context/
  ...
  react-logo.svg.png               ← React logo        →  ../react-logo.svg.png
  amazon-logo.png                  ← Amazon 225×225 colour (corner placeholder)
  amazon-logo-large.png            ← Amazon wide colour (light slides, JS-swapped)
  amazon-logo-large-white.png      ← Amazon wide white (dark slides, JS-swapped + cover)
  day-47.2/
    day47.2_carousel.html          ← GOLDEN FILE ⭐
    pic-1.png / pic-2.png / pic-3.png  ← screenshots (embed as base64)
    App.jsx / components/...       ← source files (read for code content)
```

When starting a new project, add its logos to the context root and update §6 JS swap paths and §5.2 cover img src.
...
.logo-row--dual>.logo-icon:first-child,.logo-row--dual>.logo-icon:last-child{background:transparent;border:none;box-shadow:none;}
.slide.lt .logo-row--dual>.logo-icon:first-child,.slide.lt .logo-row--dual>.logo-icon:last-child{background:transparent;border:none;}
...
### 5.2 Cover slide (GOLDEN — day47.2)

No `logo-row`. Project logo lives inside `.content`. `.pb` is a sibling of `.sp-cover`.
...
**Cover rules:**
- `.dp-row` and `.cover-taglines` → direct children of `.sp-cover` (NOT inside `.content`)
- `.content` → `h1.bh.xl` + `p.cmt` + project logo `<img>` only
- `.ibar` → direct child of `.sp-cover`, `margin-top:auto` keeps it at bottom
- `.pb` → sibling of `.sp-cover`, direct child of `.slide` (NOT inside sp-cover)
- Project logo: white version on dark cover background
- `bh xl` (34px) on cover — NOT `bh vis-xl`
- No `logo-row` of any kind on cover

---

### 5.3 Inner slide logo row

All inner slides (1 through N-2, plus CTA) use `logo-row--dual`. The second logo src is replaced by JS at load — just put the small placeholder:

```html
<div class="logo-row logo-row--dual">
  <div class="logo-icon"><img src="../react-logo.svg.png" alt="React"></div>
  <div class="logo-icon"><img src="../amazon-logo.png" alt="Project"></div>
</div>
```

JS swaps the second logo to the large colour or white version based on dark/light slide class. See §6.
  ...
  <div class="sp">
    ...
    <div class="pic-stack">
      <div class="pic-item">
        ...
        <img class="pic-img" src="BASE64_OR_PATH" alt="Screen Name">
      </div>
...
- `.pic-img` CSS: `object-fit:contain; object-position:top center; background:#d9deea` — shows full screenshot from top, fills empty area with slate blue
- Labels use `.pic-label` (9px, letter-spacing 1.5px, green)
- Light slide corner marks: `<div class="cr tl">` (no `.rc` — CSS handles colour)
...
<div class="slide [lt rlt]" id="slide-N">
  ...
  <div class="sp">
    ...
    <div class="code-block [sm]">
      <span class="file-label">filename.jsx</span><!-- optional -->
...
| `.cb-attr` | `#9cdcfe` blue | JSX/object attributes, variables |
| `.cb-cm` | `#5a7a52` grey-green | `// comments` |
| `.cb-var` | `#9cdcfe` blue | variables (same as attr) |
| `.cb-punc` | `#808080` grey | `{ } ( ) => ; , .` |
| `.cb-num` | `#b5cea8` light green | numbers |

Light slides remap all `.cb-*` colours automatically via CSS — no changes needed in the HTML.

**Code block uses `white-space:pre`** — write literal newlines in the HTML source.  
Use `<span class="code-divider"></span>` for a thin visual separator between two blocks on the same slide.
...
Update `SLIDE_COUNT`, logo swap paths, and filename prefix per day.

```javascript
const SLIDE_COUNT = 8; // ← UPDATE

const track = document.getElementById('track');
let current = 0;

// React hero atom canvas
...
// On load: draw hero + swap project logo to correct variant
  ...
  document
    ...
    .forEach((img) => {
      ...
      img.src = slide.classList.contains('lt')
        ? '../amazon-logo-large.png'        // colour for light slides
        : '../amazon-logo-large-white.png'; // white for dark slides
    });
});
...
async function captureSlide(slideEl) {
  ...
  try {
    ...
    if (hero) hero.style.filter = 'none'; // strip blur for clean canvas export
    return await html2canvas(clone, {
      width: 480, height: 640, scale: 2.25,
      useCORS: true, allowTaint: false,       // ← always false
      backgroundColor: null, scrollX: 0, scrollY: 0, logging: false
    });
  } finally {
    document.body.removeChild(wrapper);       // ← always runs, never stuck
  }
}
...
**Use `bh vis-xl` on ALL content slide headings.** Never use `bh sm` for main headings — looks undersized.

### Dark / light pattern
- Slide 0: dark (cover)
- Alternate: slide 1 dark, 2 light, 3 dark, 4 light...
- Last slide: dark (CTA)

React-themed light slides: `class="slide lt rlt"`. Plain light slides: `class="slide lt"`.

### Corner marks
| Slide type | Markup |
|------------|--------|
| Dark React slides | `<div class="cr rc tl">` (cyan corners) |
| Light slides (lt/rlt) | `<div class="cr tl">` (CSS sets blue via `.rlt`) |
| Cover | `<div class="cr tl">` (green, terminal style) |
| CTA | `<div class="cr tl">` (green) |

### Logo rules
- **Cover:** No `logo-row` at all. Project logo is a `<img>` inside `.content`, white version.
- **All other slides:** `logo-row logo-row--dual`. HTML placeholder uses `../amazon-logo.png`. JS on load swaps to large colour (light slides) or large white (dark slides).
- **Never use `logo-row--cover`** (removed in day47.2).

### Content rules
- Never add accent lines under headings
- `.prompt` at top sets the terminal context (path + command)
- `.stag` is the small category label above the heading
- Only add `.cmt` if it genuinely adds information not covered by the heading

---

## 9. Image handling

| Image | Where | Approach |
|-------|-------|----------|
| DP (cover) | `../insta-dp.jpg` | Relative path, inlined at export |
| DP (CTA) | `../insta-dp.jpg` | Relative path (or base64 — both work) |
| Project logo large (cover content) | `../project-logo-large-white.png` | Relative path |
| Project logo large (JS-swapped, inner slides) | `../project-logo-large.png` / `...-white.png` | Relative path, JS-assigned |
| Corner logos | `../react-logo.svg.png`, `../amazon-logo.png` | Relative path |
| Screenshots (visual slide) | inline in HTML | Base64 data URIs |

All relative paths are resolved via `inlineImagesForExport` using `new URL(src, window.location.href)` — requires serving from `http://localhost`.
...
def pb(i, rc=True):
    c = ' rc' if rc else ''
    pct = (i+1)/N*100
    return ('<div class="pb"><div class="pt"><div class="pf%s" style="width:%.1f%%"></div></div>'
            '<span class="pl%s">%02d&nbsp;/&nbsp;%02d</span></div>') % (c, pct, c, i+1, N)

def cr(rc=True):
    c = ' rc' if rc else ''
    return ('  <div class="cr%s tl"></div>\n  <div class="cr%s tr"></div>\n'
            '  <div class="cr%s bl"></div>\n  <div class="cr%s br"></div>') % (c,c,c,c)
...
- Use `%` formatting — not f-strings — for HTML with `{}` braces
- Build each slide as its own string variable (`s0`, `s1`…)
- JS block: use raw strings `r"""..."""` so `${...}` template literals pass through
...
- [ ] Progress bar `width:PCT%` correct for each slide
- [ ] Dot count = slide count, first dot has `class="igdot on"`
- [ ] `dl-single` button count = slide count
- [ ] Cover: no `logo-row`, has `react-hero` canvas, `.pb` is sibling of `.sp-cover`
- [ ] Inner slides: `logo-row logo-row--dual` on every slide including CTA
- [ ] Cover heading: `bh xl` (34px), all content headings: `bh vis-xl` (42px)
- [ ] No `\!` in JS — plain `!` only
- [ ] `allowTaint: false` in html2canvas
- [ ] `try/finally` on all download functions
- [ ] Logo swap JS: `.slide:not(#slide-0) .logo-row--dual .logo-icon:last-child img`
- [ ] Screenshots on visual slide: `object-fit:contain; object-position:top center; background:#d9deea`
- [ ] No accent lines under headings
- [ ] No redundant `.cmt` lines that repeat the heading
...
|------|------|-------------|
| Cover | Day hook, project, DP, atom hero | `.sp-cover`, `.dp-row`, `.cover-taglines`, `.content`, `bh xl` |
...
## 14. Project-specific (Amazon Clone — day47.2+)

**Logo paths for JS swap:**
- Dark slides → `../amazon-logo-large-white.png`
- Light slides → `../amazon-logo-large.png`

**Cover content image:** `../amazon-logo-large-white.png`, max-width 260px

**Cover ibar stack:** `react · vite · react-router-dom`

**Corner placeholder:** `../amazon-logo.png` (225×225 square)

When switching to a new project, update: JS logo swap paths, cover img src, DUAL placeholder, ibar stack line, title/igh-sub/igcap text.

---

*End of Context v7 — golden file: `day-47.2/day47.2_carousel.html`*
